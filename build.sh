...
#   ./build.sh windows    -> Windows x86_64 .exe, Release.
...
#                            into dist/. Does NOT upload anything.
...
#                            the URL. Deliberately separate from every build
#                            command: uploading publishes, and that should be
...
build_windows() {
  ...
  "$LAZBUILD" --os=win64 --cpu=x86_64 --ws=win32 \
              --build-mode="$MODE_SHIP" --build-all "$PROJ"
...
do_dist() {
  ...
  ls -la "$DIST" | grep "$s"
  echo
  echo "to share one:  ./build.sh upload dist/heckers-sketch-$s-win64.exe"
}
...
case "${1:-}" in
  ...
  all)            build_linux "$MODE_DEV"; build_windows ;;
  run)            build_linux "$MODE_DEV"; say "running"; exec "$ROOT/$APP" ;;
  clean)          do_clean ;;
  fresh)          do_clean; build_linux "$MODE_DEV"; build_windows ;;
  dist)           do_dist ;;
