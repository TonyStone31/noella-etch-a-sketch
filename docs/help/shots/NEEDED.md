...
* **Walk the pointer, do not jump it.**  hidctl's `at` is a teleport, and the
  whole point of a moving picture is seeing where the hand went.
  `tools/gif-push.txt` was generated by a few lines of Python that emit a
  chain of `at` and short `wait` along an eased path; the comment at the top
  of it says so.
* **It plays back faster than it records.**  hidctl has its own overhead on
  every step, so a script that reads as nine seconds records as sixteen.
  `SPEED` in `gif-shot.sh` is what fixes that; `WIDE`, `FPS` and `COLORS` are
  what keep the file small enough to sit on a page.
