...
iso paper: a run of legs along the three axes, each with its centre-to-centre
...
the leg, type its centre-to-centre length in inches, press Enter.  The next
...
* **Cut lengths**: centre to centre less the take-outs at each end.  That is
...
The centreline is the straight legs with the bends drawn in as arcs, and
...
its centre-to-centre length, its cut length, and the elbow after it.  Then
the count of 90s and 45s, the flanges, and the centre-to-centre total.
